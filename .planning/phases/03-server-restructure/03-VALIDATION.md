...
| 3-03-03 | 02 | 2 | SERV-02 | manual | `curl -X POST http://localhost:3001/api/find-email -H 'Content-Type: application/json' -d '{}'` → 400 `{ error, message, statusCode }` | ❌ W0 | ⬜ pending |
| 3-03-04 | 02 | 2 | SERV-02 | manual | `curl -X POST http://localhost:3001/api/suggest-domains -H 'Content-Type: application/json' -d '{}'` → 400 `{ error, message, statusCode }` | ❌ W0 | ⬜ pending |
| 3-03-05 | 02 | 2 | SERV-02 | manual | `curl -X POST http://localhost:3001/api/draft-email -H 'Content-Type: application/json' -d '{}'` → 400 `{ error, message, statusCode }` | ❌ W0 | ⬜ pending |
...
- [x] `server/lib/` directory — must exist before `prisma.js` is created
- [x] `server/lib/prisma.js` — Prisma singleton (infrastructure prerequisite for all route files)
- [x] `server/routes/` directory — must exist before route files are created
...
| POST /api/find-email with missing company returns 400 | SERV-02 | No test suite in Phase 3 | `curl -X POST http://localhost:3001/api/find-email -H "Content-Type: application/json" -H "x-secret: $SECRET" -d '{}'` → `{ "error": "Validation Error", "message": "...", "statusCode": 400 }` |
| POST /api/suggest-domains with missing company returns 400 | SERV-02 | No test suite in Phase 3 | `curl -X POST http://localhost:3001/api/suggest-domains -H "Content-Type: application/json" -H "x-secret: $SECRET" -d '{}'` → `{ "error": "Validation Error", "message": "...", "statusCode": 400 }` |
| POST /api/draft-email with missing draftType returns 400 | SERV-02 | No test suite in Phase 3 | `curl -X POST http://localhost:3001/api/draft-email -H "Content-Type: application/json" -H "x-secret: $SECRET" -d '{}'` → `{ "error": "Validation Error", "message": "...", "statusCode": 400 }` |
