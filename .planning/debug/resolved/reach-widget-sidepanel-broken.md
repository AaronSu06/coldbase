...
hypothesis: CONFIRMED AND RESOLVED — see Resolution section.
test: n/a
expecting: n/a
next_action: n/a — debug session archived
...
root_cause: |
  logger.js contained `export { makeLogger as logger }` — an ES module export statement.
  Content scripts are loaded as CLASSIC scripts, not ES modules. Any file with export/import
  syntax throws an immediate SyntaxError when loaded as a classic script.

  With logger.js throwing a SyntaxError on load, window.ReachLogger was never set.
  Every downstream content script (email-detector.js, compose-widget.js, tracking.js,
  content.js) crashed on its first line with "window.ReachLogger is not a function".
  The entire extension was dead on load — no widget, no sidebar, nothing.

  background.js had a separate but related issue: it imported `{ logger }` from logger.js
  using ES module import syntax. Since logger.js has no exports, this would have caused
  a "does not provide an export named 'logger'" error in the service worker as well.

fix: |
  commit: 5b2155a  fix(04-01): remove ES module export from logger.js

  1. extension/logger.js: Removed the export statement entirely. logger.js is now a
     pure classic script that only sets window.ReachLogger = makeLogger. No export/import
     syntax anywhere in the file.

  2. extension/background.js: Removed `import { logger } from './logger.js'`. background.js
     (an ES module) defines makeLogger inline — identical implementation, no file dependency.

  3. All [REACH-DIAG] diagnostic console.log lines removed from email-detector.js,
     compose-widget.js, content.js, and logger.js.

  4. The null guards in compose-widget.js (updateWidget, getOrCreateWidget) and the
     defensive try/catch wrappers in content.js around each init() call are KEPT — they
     are valid defensive code regardless of the logger.js bug.

verification: |
  Fix confirmed via prior checkpoint: console showed the exact SyntaxError on logger.js
  and the cascade of "window.ReachLogger is not a function" on every content script.
  Fix addresses the root cause directly. User instructed to reload the extension and test.
